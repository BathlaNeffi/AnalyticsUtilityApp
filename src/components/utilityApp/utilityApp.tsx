...
const UtilityApp:React.FC= () => {
  ...
  return (
    ...
    <button style={{cursor:"pointer"}} onClick={()=>setOpenFlavanoids(!openFlavanoids)}>{openFlavanoids?"Close Flavanoids":"Flavanoids"} </button>
    <button style={{cursor:"pointer"}} onClick={()=>setOpenGamma(!openGamma)}>{openGamma?"Close Gamma":"Gamma"}</button>
    </div>
